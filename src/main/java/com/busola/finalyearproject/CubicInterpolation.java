...
public class CubicInterpolation {
    ...
    public void run1(Function f, double x1, double x2, double x3) {
        this.run1(f, x1, x2, x3, 1e-5);
    }
    
    public void run1(Function f, double x1, double x2, double x3, double tol) {
        System.out.println("\nCubic Intepolation >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
        Stopwatch timer = new Stopwatch();
